...
class OAT(object):
    def __init__(self, OAFile=os.path.split(os.path.realpath(__file__))[0] + '/data/ts723_Designs.txt'):
        ...
        with open(OAFile, ) as f:
            ...
            self.data[key] = dict(pos=pos,
                                  ...
                                  mk=[[int(mk.split('^')[0]), int(mk.split('^')[1])]for mk in key.split('n=')[0].strip().split(' ')],
                                  data=value)
        self.data = sorted(list(self.data.items()), key=lambda i: i[1]['pos'])
    ...
    def genSets(self, params, mode=0, num=1):
        ...
        """
        sets = []
        mk = [(k, len(list(v)))for k, v in groupby(list(params.items()), key=lambda x:len(x[1]))]
        data = OAT.get(self, mk)
        for d in data:
            # 根据正则表结果生成测试集
            q = OrderedDict()
            for index, (k, v) in zip(d, list(params.items())):
                try:
                    q[k] = v[index]
                except IndexError:
                    # 参数取值超出范围时，取None
            ...
            if q not in sets:
                if mode == 0:
                    sets.append(q)
                elif mode == 1 and (len([v for v in list(q.values()) if v is None])) <= num:
                    # 测试集裁剪,去除重复及含None测试集
                    sets.append(q)
        return sets
