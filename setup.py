    ...
    try:
        ...
        if not modules:
            ...
            if 'security' in modules:
                modules.append('pycryptodome')
            if 'db' in modules:
                modules.append('sql')
            if 'webservice' in modules:
                modules.append('suds-jurko')
            for tfile in [init_file, requirements_file]:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        ...
        if not sys.argv[2:]:
            ...
            print('{0} not all in the supported modules list {1}'.format(
                sys.argv[2:], all_modules))
        ...
        print(
            '\t-w module A [module B...], --without module A [module B...]\tsetup without the modules, eg "--without redis security db".')
